...
class TektiteVideoCombiner3:
    ...
    def INPUT_TYPES(cls):
        ...
        # Optional gate: if connected, combiner will not start waiting/stitching
        # until this input is available from upstream.
        optional_inputs["start_signal"] = ("*", {"lazy": True})
    ...
    def check_lazy_status(self, start_signal=None, **kwargs):
        # Only enforce lazy dependency when start_signal is actually wired.
        # If it's not wired, Comfy typically does not pass it in kwargs.
        if "start_signal" in kwargs and start_signal is None:
            return ["start_signal"]
        return []
    ...
    def combine_videos(
        ...
        **kwargs,
        ...
        while time.time() < deadline:
            ...
            raise ValueError(
                f"Tektite Video Combiner 3.0 timed out after {wait_timeout_sec}s while waiting. Last state: {last_state}"
            )
        print(
            f"[Tektite Video Combiner 3.0] Ready {expected_clips}/{expected_clips}. Starting normalize+stitch."
        )
    ...
    def _build_progress_line(
        ...
        *,
        ...
        return (
            f"[Tektite Video Combiner 3.0] Clips ready: {ready_clips}/{expected_clips} | "
    ...
    def _ensure_writable_output_path(self, resolved_output: str) -> str:
        ...
        print(
            f"[Tektite Video Combiner 3.0] Output directory not writable: {out_dir}. "
...
NODE_CLASS_MAPPINGS = {
    "TektiteVideoCombiner3": TektiteVideoCombiner3,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "TektiteVideoCombiner3": "Tektite Video Combiner 3.0",
}
